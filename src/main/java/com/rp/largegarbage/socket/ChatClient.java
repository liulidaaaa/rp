...
public class ChatClient {
    ...
    public static void connect() throws IOException {
        ...
        bufferedReader = new BufferedReader(new InputStreamReader(socket.getInputStream(), "GBK"));//缓冲区
    ...
    static void doCommand(JSONObject jsonObject) {
        //解析 header body
            ...
            case "8000":
                ...
                switch (cmd1) {
                    ...
                    case "1002":
                        ...
                        System.out.println("===============返回心跳 "+recieveHeartPackTime+"=============");
                        break;
                    default:
                }
                break;
            ...
            while (true) {
                ...
                if (l > 2) {
                    ...
                    if (!socket.isConnected()) {
                        try {
                            LOGGER.info("2分钟没有接收到心跳包的返回，重连");
                            connect();
                        } catch (IOException ioException) {
                            ioException.printStackTrace();
                        }
                        if (socket.isConnected()) {
                            LOGGER.info("2分钟没有接收到心跳包的返回，重新登录");
                            login();
                            try {
                                recieveMessage();
                            } catch (IOException e) {
